...
def first(request):
    ...
    return render(request,'index1.html', {'result': user})
...
def index(request):
    ...
    #return render(request,'index1.html',{'result':data})
...
def adduser(request):
    if request.method=="POST":
        ...
        cus.save()
        return render(request,'index1.html', {'message1':'successfully Registered'})
...
def addworker(request):
    if request.method=="POST":
        ...
        cus=wrkregg(name=name,file=myfile,email=email,gender=gender,labour_category=labour_category,district=district,labour_type=labour_type,state=state,centre=centre,phone=phone,category=category,password=password,status=status)
        cus.save()
    return render(request,'index1.html', {'message1':'successfully Registered'})
...
def addlogin(request):
    ...
    if email == 'admin@gmail.com' and password =='admin':
        ...
        if userdetails.password == request.POST['password']:
            ...
            request.session['wdis'] = userdetails.district

            request.session['wemail'] = email
            request.session['category'] = userdetails.category
            request.session['place'] = userdetails.centre
...
def pred(request):
    if request.method=='POST':
       ...
       result=wage_pred.predict(centre,gender,labour_category,labour_type,state,district,month,year)
...
def wapprove(request,id):
    ...
    h=sel.district
    i=sel.gender
    j=sel.labour_category
    k=sel.labour_type
    l=sel.state
    m=sel.file
    upd=wrkregg(name=a,email=b,phone=e,centre=g,district=h,gender=i,labour_category=j,file=m,state=l,labour_type=k,password=c,category=f,status='approved',id=id)
    upd.save()
...
def addbook(request):
    if request.method=="POST":
        ...
        user = category.objects.all()
       # return redirect(viewwrkrs)
    return render(request,'index1.html', {'message2':'successfully Booked','result':user})
...
def wprofile(request):
    #a= request.session['uid']
    sel=wrkregg.objects.get(id=request.session['wid'])
    return render(request,'workerprf.html', {'sel':sel})
...
def confirm(request,id):
    ...
    h=sel.no_wrkr
    #i=sel.rate
    upd=booking(uname=a,wid=b,place=d,phone=e,date=c,typee=f,issue=g,no_wrkr=h,status='confirmed',id=id)
    upd.save()
...
def reject(request,id):
    ...
    h=sel.no_wrkr
   # i=sel.rate
    upd=booking(uname=a,wid=b,place=d,phone=e,date=c,typee=f,issue=g,no_wrkr=h,status='rejected',id=id)
    upd.save()
    return redirect(v_book)

def report(request):
    a=request.session['wname']
    data=booking.objects.filter(status='confirmed',wid=a)
...
def addpayment(request):
    if request.method=="POST":
        ...
        return render(request,'index1.html', {'message':'successfully paid','result':user})
